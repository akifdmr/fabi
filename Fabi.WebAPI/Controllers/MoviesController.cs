namespace Fabi.WebAPI.Controllers;

[Route("api/[controller]")]
[ApiController]
public class FabiController : ControllerBase
{
    #region Props
    ...
    public FabiController(IUnitOfWork unitOfWork,
    ...
    {
        var Fabi = await _unitOfWork.Movie.GetAllAsync(
            includeProperties: "Genre", 
            orderBy: m => m.Rate,
            orderByDirection: SD.Descending);

        return Ok(_mapper.Map<IEnumerable<MovieDetailsDto>>(Fabi));
    }
    ...
    public async Task<IActionResult> CreateAsync([FromForm] MovieCreateionDto dto)
    {
        ...
        //var x = 5;
        string posterUrl = await _fileHandler.Image.Upload(dto.Poster, Path.Combine(_hostEnviroment.WebRootPath, SD.FabiPosterpath));
    ...
    public async Task<IActionResult> UpdateAsync(int id, [FromForm] MovieUpdateDto dto)
    {
        ...
        if (dto.Poster is not null)
        {
            ...
            //upload new poster
            var newPosterUrl = await _fileHandler.Image.Upload(dto.Poster, Path.Combine(_hostEnviroment.WebRootPath, SD.FabiPosterpath));
